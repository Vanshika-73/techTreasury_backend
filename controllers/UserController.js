...
const userRegister = asyncHandler(async (req, res, next) => {
  try {
    ...
    const { _id, name, email, role } = await User.create(req.body);
    res.status(200).send({
      _id,
      name,
      email,
    ...
    console.error('Error caught in userRegister:', error);
    next(error);
  }
});
...
const updateUser = asyncHandler(async (req, res, next) => {
  try {
    ...
    res.status(200).send({ _id,name, email, role, token: generateToken(_id), });
  } catch (error) {
    next(error);
  }
});
...
const updateAdmin = asyncHandler(async (req, res, next) => {
  try {
    ...
    res.status(200).send({ _id, name, email, role });
  } catch (error) {
    next(error);
  }
});
